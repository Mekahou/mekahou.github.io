...
%=====================================================================
\section{\textcolor{KahouBlack}{What economists mean by ``solving a model''}}
%=====================================================================

\begin{frame}{An economic model is a system of functional equations}
	\begin{itemize}
		\item Economists build models by specifying how agents behave: what they want (preferences), what they can do (technology), and what limits them (constraints).
		\vspace{0.1in}
		\item Optimizing behavior implies \emphcolor{functional equations whose unknowns are functions}.
		\vspace{0.1in}
		\item \emphcolor{Example:} a social planner chooses a consumption rate $c_t$ given capital $k_t$ and stochastic productivity $z_t$. Capital evolves as:
		\begin{align*}
			\dot{k}_t = e^{z_t} k_t^{\alpha} - \delta k_t - c_t
		\end{align*}
		Output $e^{z_t} k_t^\alpha$ is produced, capital depreciates at rate $\delta$, and whatever is not consumed is invested.
		\vspace{0.05in}
		\item Productivity follows an Ornstein--Uhlenbeck process:
		\begin{align*}
			dz_t = -\mu z_t\, dt + \sigma\, dW_t
		\end{align*}
		\item The social planner maximizes expected discounted lifetime utility:
		\begin{align*}
...
\begin{frame}{What does ``solving'' this model mean?}
	\begin{itemize}
		\item The problem has a recursive representation in the form of a PDE, the \emphcolor{Hamilton--Jacobi--Bellman equation}:
		\begin{empheq}[box=\tcbhighmath]{align*}
			\rho V = \max_c \left\{ u(c) + V_k\!\left[e^z k^\alpha - \delta k - c\right] - \mu z\, V_z + \tfrac{1}{2}\sigma^2 V_{zz} \right\}
		\end{empheq}
		\item The unknown is the \emphcolor{value function} $V(k,z)$:
		\begin{enumerate}
			\item The first-order condition $u'(c) = V_k(k,z)$ gives the optimal policy $c = (u')^{-1}(V_k)$.
			\item From the capital equation, the drift is $\dot{k} = e^z k^\alpha - \delta k - (u')^{-1}(V_k)$.
		\end{enumerate}
		\item ``Solving the model'' means finding $V(\cdot)$ that satisfies the HJB equation over the state space.
		\item In general, no closed-form expression exists.
		\item An alternative is to solve the Euler equation:
		\begin{align*}
			\frac{d\left[u'(c_t)\right]}{u'(c_t)} = \left[\rho - \left(\alpha e^{z_t} k_t^{\alpha-1} - \delta\right)\right] dt + \sigma\, \frac{u''(c_t)}{u'(c_t)}\, d_z(k_t,z_t)\, dW_t
		\end{align*}
	\end{itemize}
\end{frame}

\begin{frame}{The mathematical structure you already know}
	\begin{itemize}
		\item Rewrite the HJB equation as:
		\begin{align*}
			\Op(V) \equiv \rho V - \max_c \left\{ u(c) + V_k\!\left[e^z k^\alpha - \delta k\right] - \mu z\, V_z + \tfrac{1}{2}\sigma^2 V_{zz} \right\} = 0
		\end{align*}
		\item Strip away the economics, and you are looking for a function $V$ such that:
		\begin{empheq}[box=\tcbhighmath]{align*}
			\Op(V) = \mathbf{0}
		\end{empheq}
		where $\Op$ is a nonlinear operator between function spaces. Nothing exotic here.
		\item The HJB equation is a second-order nonlinear PDE. You have seen these in optimal control and differential games.
		\item The classical numerical approach is exactly what you would do:
		\begin{enumerate}
			\item Propose a parameterized approximation $\widehat{V}_\theta$.
			\item Evaluate the residual $\Resid[\widehat{V}_\theta](X)$ at a collection of points.
			\item Find $\theta$ that drives the residual to zero.
		\end{enumerate}
	\end{itemize}
\end{frame}

\begin{frame}{So what makes the economics problem different?}
	\begin{itemize}
		\item When you solve a typical PDE, you know the equation before you write a single line of code. The coefficients, the forcing term, the boundary conditions: all given.
		\item You can pick your grid, choose your discretization, and refine your mesh. The equation never changes on you. Better numerics give a better approximation to a \emphcolor{fixed target}.
		\item In economics, this is not the case. The law of motion for the state variables \emph{is} the policy function we are trying to find. Agents observe the current state, make a choice, and that choice determines where the system goes next.
		\item So improving your approximation changes the dynamics, which changes which parts of the state space the system visits, which changes where your approximation needs to be accurate.
	\end{itemize}
	\begin{block}{The difference in one sentence}
		In typical natural science problems, the dynamics are an input. In economics, the dynamics are the output.
	\end{block}
\end{frame}

\begin{frame}{The equilibrium loop}
	\begin{itemize}
		\item Return to the HJB equation:
		\begin{align*}
			\rho V = u\big((u')^{-1}(V_k)\big) + V_k\!\left[e^z k^\alpha - \delta k - (u')^{-1}(V_k)\right] - \mu z\, V_z + \tfrac{1}{2}\sigma^2 V_{zz}
		\end{align*}
		\vspace{0.05in}
		\item We already mentioned this is a second-order nonlinear PDE.
		\item But look at the drift term for $k$: \ $e^z k^\alpha - \delta k - (u')^{-1}(V_k)$.
		\item The law of motion for the state depends on $V$, the very function we are solving for!
		\vspace{0.05in}
		\item \emphcolor{The equilibrium loop:} the ``coefficients'' of the PDE are endogenous to the optimal choices of the agents.
	\end{itemize}
\end{frame}

\begin{frame}{Why it matters}
	\begin{itemize}
		\item Since the drift depends on $V$, we need to choose \emph{where} to evaluate the residual.
		\item A natural choice: weighting the residual by the \emphcolor{ergodic distribution} $\xi^*$:
		\begin{align*}
			\theta^* = \arg\min_\theta \int \Resid[\widehat{V}_\theta](X)^2\, d\xi^*(X)
		\end{align*}
		\item But $\xi^*$ is generated by the law of motion, which depends on $V$, the function we are solving for:
		\begin{align*}
		...
		\end{align*}
		\item Improving $\widehat{V}_\theta$ changes the drift, changes $\xi^*$, and changes where the nodes should have been. Moving the net moves the target.
	\end{itemize}
	\begin{block}{Practical fix}
		Iterate between sampling and optimization. Simulate from the current $\widehat{V}_\theta$, recompute evaluation points, re-optimize. Convergence is not guaranteed but works well in practice.
	\end{block}
\end{frame}

\begin{frame}{Why traditional methods hit a wall}
	\begin{itemize}
		\item When the state space is low-dimensional ($N \le 4$), everything we just described works fine: lay down a grid or use Chebyshev collocation and solve.
		\item But many of the questions economists want to answer require large $N$:
		\begin{itemize}
			\item Heterogeneous households differing in wealth, income, age, and portfolio composition.
			\item Firms interacting strategically in a dynamic oligopoly.
			\item Spatial models with many regions linked by trade and migration.
		\end{itemize}
		\item Grid-based methods need $m^N$ points. Chebyshev collocation with tensor products needs $D^N$ terms. Both become infeasible beyond $N = 5$ or so.
		\item Perturbation (Taylor expansion around steady state) sidesteps the grid but is only locally accurate. It fails with kinks, occasionally binding constraints, or large shocks.
	\end{itemize}
\end{frame}

\begin{frame}{Enter neural networks (the part you know)}
	\begin{itemize}
		\item The idea is simple: replace the fixed polynomial basis with a neural network:
		\begin{empheq}[box=\tcbhighmath]{align*}
			\widehat{d}_\theta(X) = f_\theta(X), \qquad \theta^* = \arg\min_\theta \frac{1}{L}\sum_{l=1}^{L} \Resid[f_\theta](X_l)^2
		\end{empheq}
		\item You already know why this helps: universal approximation, dimension-free rates for the Barron class (\textcolor{emphcolorval}{Barron, 1993}), learned representations. I will take such background as given.
		\item What is specific to our setting:
		\begin{enumerate}
			\item The residual $\Resid$ is not a supervised-learning loss. It comes from the model's equilibrium conditions (Bellman, Euler, market clearing).
			\item Evaluation points $\{X_l\}$ are regenerated each epoch from the current approximation (the equilibrium loop we just discussed).
			\item There may be additional constraints: transversality conditions, non-negativity of consumption, budget balance.
		\end{enumerate}
	\end{itemize}
\end{frame}

%=====================================================================
\section{\textcolor{KahouBlack}{The blessings of overparameterization}}
%=====================================================================

\begin{frame}{The question this paper asks}
	\begin{itemize}
		\item Now that you know the setup (finding functions that satisfy operator equations, with the complication that the sampling measure is endogenous), a practical question arises:
		\vspace{0.05in}
		\item \emphcolor{How large should the network be?}
		\vspace{0.05in}
		\item Standard statistical wisdom says: more parameters than evaluation points $\Rightarrow$ overfitting $\Rightarrow$ poor out-of-grid performance.
		\item If true here, the practitioner must carefully tune network width, adding a fragile hyperparameter choice to an already demanding computational problem.
	\end{itemize}
	\begin{block}{Thesis}
		This paper argues the opposite. In the context of solving these operator equations, overparameterization is a \emphcolor{blessing}. It improves both accuracy and, more importantly, algorithmic stability.
	\end{block}
\end{frame}

\begin{frame}{Two findings}
	\begin{itemize}
		\item \emphcolor{Finding 1, no overfitting:} as the number of network parameters grows, out-of-grid residuals (HJB and Euler) \emph{decrease}. The approximate value and policy functions converge toward their benchmark counterparts.
		\item \emphcolor{Finding 2, algorithmic stability:} with small networks, solutions obtained from different random initializations of $\theta$ disagree substantially.
		\begin{itemize}
			\item The researcher has \emphcolor{no way of knowing} which initialization produced the better solution. The ``answer'' depends on the random seed.
			\item As network width increases, this disagreement collapses: 50 independent random seeds converge to the same function.
		\end{itemize}
	\end{itemize}
...
\begin{frame}{Connection to the machine learning literature}
	\begin{itemize}
		\item Our findings connect to a broader phenomenon in modern machine learning.
	\end{itemize}
	\vspace{0.1in}
	\begin{columns}
		\begin{column}{0.5\textwidth}
			\begin{block}{Double descent}
				\textcolor{emphcolorval}{Belkin et al. (2019)}: the classical bias--variance tradeoff breaks down for overparameterized models. Test error can decrease \emph{again} after an initial overfitting peak.
			\end{block}
		\end{column}
		\begin{column}{0.5\textwidth}
			\begin{block}{Benign overfitting}
				\textcolor{emphcolorval}{Bartlett et al. (2020)}: minimum-norm interpolants can generalize well even when parameters vastly exceed observations.
			\end{block}
		\end{column}
	\end{columns}
	\vspace{0.15in}
	\begin{itemize}
		\item \emphcolor{Our contribution:} this benign behavior extends to function approximation problems in computational economics, where the loss is defined by \emphcolor{equilibrium conditions} (HJB, Bellman, or Euler equations) rather than by labeled data.
	\end{itemize}
\end{frame}
...
\begin{frame}{Experimental design}
	...
	\vspace{0.15in}
	\begin{itemize}
		\item In all three: accuracy and stability improve with width. No sign of overfitting.
		\item Theorem? So far, just a conjecture.
	\end{itemize}
\end{frame}

\begin{frame}{Network architecture and training protocol}
	\begin{itemize}
		\item \emphcolor{Hidden width} $H$ varied systematically.
		\item One hidden layer: $f_\theta(x) = W^{(2)} \sigma\!\left(W^{(1)}x + b^{(1)}\right) + b^{(2)}$, with $H(n+2)+1$ parameters.
		\item Two hidden layers: $f_\theta(x) = W^{(3)} \sigma\!\left(W^{(2)} \sigma\!\left(W^{(1)}x + b^{(1)}\right) + b^{(2)}\right) + b^{(3)}$, with $H(n+H+3)+1$ parameters.
		\item \emphcolor{Activation functions:} ReLU $\sigma(z) = \max\{0,z\}$ (baseline), Leaky ReLU $\sigma(z) = \max\{\alpha z, z\}$ with $\alpha = 0.01$, and Sigmoid $\sigma(z) = 1/(1+e^{-z})$.
		\item \emphcolor{Training:} minimize squared residual loss (Euler or Bellman) using Adam.
		\item \emphcolor{Key design choice:} 50 independent random seeds per specification. This lets us measure both accuracy (median across seeds) and stability (width of the 10th--90th percentile band).
	\end{itemize}
\end{frame}
...
\begin{frame}{LQ: deep learning implementation}
	\begin{itemize}
		\item \emphcolor{Architecture:} one-hidden-layer ReLU networks for value and policy functions. Hidden units $H = 1$ to $H = 7$ (4 to 22 parameters). 50 seeds per configuration.
		...
		\end{empheq}
		The value function is then trained on a $12 \times 12$ Cartesian grid of $(y,Y)$ pairs. Adam optimizer with early stopping at loss $< 10^{-8}$.
...
\begin{frame}{LQ: training and test data}
	% NOTE: scatter figure (12x12 train grid vs T=29 test path) not among uploaded
	% figures; add the file to figures/ and \includegraphics here when available.
	\begin{itemize}
		\item Red dots: \textcolor{emphcolorval}{$12 \times 12$} training grid. Blue crosses: \textcolor{emphcolorval}{$T = 29$} test path simulated from fixed initial conditions.
		\item Test points lie strictly in the interior of the state space and never coincide with a training node.
	\end{itemize}
\end{frame}
...
\begin{frame}{LQ: absolute relative error vs.\ closed-form solution}
	\begin{figure}[t!]
		\centering
		\includegraphics[width=0.85\textwidth]{figures/LQ_policy_rel_error_median_vs_max.pdf}
	\end{figure}
	\begin{itemize}
...
\begin{frame}{LQ: value function results}
	\begin{columns}
		\begin{column}{0.5\textwidth}
			\begin{center}\footnotesize (A) Train and test Bellman MSE\end{center}
			\includegraphics[width=\textwidth]{figures/LQ_value_test_train_loss.pdf}
		\end{column}
		\begin{column}{0.5\textwidth}
			\begin{center}\footnotesize (B) Median and max relative error\end{center}
			\includegraphics[width=\textwidth]{figures/LQ_value_rel_error_median_vs_max.pdf}
		\end{column}
	\end{columns}
	\vspace{0.1in}
	\begin{itemize}
		\item Value function network (trained conditional on estimated policy) displays the same pattern: accuracy improves and cross-seed uncertainty collapses with width.
	\end{itemize}
\end{frame}
...
\begin{frame}{From a linear to a non-smooth operator equation}
	\begin{itemize}
...
\begin{frame}{McCall: deep learning implementation}
	\begin{itemize}
...
\begin{frame}{McCall: train and test Bellman-residual loss}
	\begin{columns}
		...
		\begin{column}{0.45\textwidth}
			\begin{itemize}
				\item Train loss hits the early-stopping floor at the interpolation threshold ($\sim$12 parameters, matching $N = 12$ training points).
...
\begin{frame}{McCall: absolute relative error}
	\begin{figure}[t!]
		\centering
		\includegraphics[width=0.85\textwidth]{figures/Mccall_rel_error_median_vs_max.pdf}
	\end{figure}
	\begin{itemize}
...
\begin{frame}{Two PDEs, one key difference}
	\vspace{0.1in}
	\begin{itemize}
		\item The time-dependent \emphcolor{Schr\"odinger equation} for a single particle in one dimension:
		\begin{align*}
			i\hbar \frac{\partial \psi}{\partial t} = -\frac{\hbar^2}{2m} \frac{\partial^2 \psi}{\partial x^2} + V(x)\, \psi
		\end{align*}
		Unknown: $\psi(x,t)$. The potential $V(x)$ is given.
		\vspace{0.1in}
		\item The \emphcolor{Hamilton--Jacobi--Bellman equation} for the stochastic neoclassical growth model:
		\begin{align*}
			\rho V = \max_c \left\{ u(c) + V_k\!\left[e^z k^\alpha - \delta k - c\right] - \mu z\, V_z + \tfrac{1}{2}\sigma^2 V_{zz} \right\}
		\end{align*}
