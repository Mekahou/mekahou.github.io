...
\begin{document}
\title{{\vspace{0.4in}\hspace{0.2in}\textcolor{KahouBlack}{Solving Models of Economic Dynamics with \\\quad Ridgeless Kernel Regressions}}}
...
\begin{frame}{Contribution}
 Using kernel methods to solve a class of infinite-horizon, deterministic, continuous-time models
\begin{enumerate}
	...
	\end{itemize}
	\vspace{0.1in}
	\item \emphcolor{Learning the correct set of steady-states}:
	\begin{itemize}
	...
	\end{itemize}
	\vspace{0.1in}
	\item \emphcolor{Consistency of the kernel estimates}.
...
\begin{frame}{Method: approximation}
	...
	\item $\balpha^x_j$, $\balpha^\mu_j$, $\balpha^y_j$, $\bmu_0$, and $\by_0$ are parameters to be found.
	 \vspace{0.1in}
	\item $k(t,t_j)$ is a kernel that measures how close (or similar) $t$ is to $t_j$.
	\vspace{0.1in}
...
\begin{itemize}
	...
	\item TVC is \emphcolor{not imposed}.
		\vspace{0.1in}
	\item The minimization term (objective) is used to control the smoothness of the approximating functions.
	\vspace{0.1in}
		...
		\begin{column}{0.5\textwidth}
			% Content for the left column
				\begin{itemize}
					...
					\begin{itemize}
						\item For short- and medium-run accuracy, we do not need to know the steady state (global condition).
						\vspace{0.1in}
						\item It suffices that sub-optimal paths diverge from the optimal path (local condition).
					\end{itemize}
					\vspace{0.1in}
	...
	\begin{columns}
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				...
				\emphcolor{Conclusion}
				\begin{itemize}
					\item For short-run accuracy, we do not need to use large $T$ (global condition).
					\vspace{0.1in}
...
\begin{frame}{Neoclassical Growth Model: Non-Concave Production Function}
	\begin{itemize}
		...
		\end{align*}
		\vspace{0.1in}
		\item The production function has a kink.	
	\end{itemize}
...
\begin{frame}{Non-concave production function: vector field}
	\begin{columns}
		\begin{column}{0.5\textwidth}
			...
			\]
			\begin{itemize}
				\item The model is identical to the concave case, except that $f(\cdot)$ is now non-concave.
				\item  This problem is very challenging for traditional methods such as shooting.
...
\begin{frame}{Results}
	...
	\begin{figure}[t!]
		\centering
		\includegraphics[width=0.6\textwidth]{figs/neoclassical_growth_model_concave_convex_threshold.pdf}
		\vspace{-4mm}
	\end{figure}
	\begin{itemize}
...
\begin{frame}{Why do we need the boundary condition?}
	...
	\begin{itemize}
		\item The solutions: 
		\begin{align*}
			\mu(t) = \mu_f(t) + \zeta e^{rt}
		\end{align*}
		\item $\mu_f(t) = \int_0^\infty e^{-r\tau} x(t+\tau)d\tau$: price based on the fundamentals.
		\vspace{0.1in}
		\item $\zeta e^{rt}$: explosive bubble terms, it has to be \emphcolor{ruled out} by the boundary condition.
		\item The price based on the fundamentals is the \emphcolor{``smoothest"}.
	\end{itemize}
\end{frame}
...
\begin{frame}{Results}
	\begin{figure}[t!]
		\centering
		\includegraphics[width=0.6\textwidth]{figs/asset_pricing_contiguous.pdf}
...
\begin{frame}{Conclusion}
	\begin{itemize}
		...
		\vspace{0.1in}
		\item The minimum-norm kernels aligns with optimality in economic dynamic models.
		\vspace{0.1in}
		\item The minimum-norm kernels accurately learn the correct steady state(s).
		\vspace{0.1in}
	...
	\end{itemize}
\end{frame}
\begin{frame}{Conclusion: explicit and implicit Regularization}
	Machine learning methods:
	\[
	\begin{aligned}
		\underbrace{\frac{1}{N}\sum_{i=1}^N \mathcal{L}\big(y_i,f_\theta(x_i)\big)}_{\text{loss}}
		\;+\;
		\underbrace{\lambda \,\Omega(f_\theta)}_{\text{regularization term}}
	\end{aligned}
	\]
	\begin{itemize}
		\item $f_\theta(\cdot)$: parametric function (kernel methods, neural networks, etc.).
		\item In this paper, regularization is used to rule out sub-optimal solutions and enforce stability.
		\item What other roles can regularization play in solving economic models with modern ML methods?
	\end{itemize}
\end{frame}
...
\begin{frame}{Human capital and growth}
	...
	\[
\begin{aligned}
	\dot{x}_k(t) &= y_k(t) - \delta_k x_k(t),\\
	\dot{x}_h(t) &= y_h(t) - \delta_h x_h(t),\\
	\dot{\mu}_k(t) &= r \mu_k(t) - \mu_k(t) \bigl[ f_1(x_k(t),\,x_h(t)) - \delta_k \bigr],\\
	\dot{\mu}_h(t) &= r \mu_h(t) - \mu_h(t) \bigl[ f_2(x_k(t),\,x_h(t)) - \delta_h \bigr],\\
	0 &= \mu_k(t) y_c(t) - 1,\\
	0 &= \mu_k(t) - \mu_h(t),\\
	0 &= f(x_k(t),\,x_h(t)) - y_c(t) - y_k(t) - y_h(t),\\
	0 &= \lim_{t\rightarrow \infty} e^{-rt} x_k(t) \mu_k(t),\\
	0 &= \lim_{t\rightarrow \infty} e^{-rt} x_h(t) \mu_h(t),
\end{aligned}
\]
for given initial conditions $x_k(0) = x_{k_0}$, $x_h(0) = x_{h_0}$.

\begin{itemize}
	\item Human capital is $x_h(t)$, physical capital $x_k(t)$, consumption $y_c(t)$, investment in human capital $y_{h}(t)$, and investment in physical capital $y_{k}(t)$,  $\mu_k(t)$ and $\mu_{h}(t)$ are the co-state variables.
\end{itemize}
...
\begin{frame}{Results}
		\begin{figure}[t!]
		\centering
		\includegraphics[width=0.4\textwidth]{figs/human_capital.pdf}
		\vspace{-4mm}
	\end{figure}
\end{frame}
\end{document}
