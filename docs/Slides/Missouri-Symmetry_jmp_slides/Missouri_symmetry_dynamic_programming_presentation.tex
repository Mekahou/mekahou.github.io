...
\begin{frame}
	...
	Quick \emphcolor{review} of collocation methods:
...
%%%%%%%%%%%
	...
	\begin{frame}{Investment under uncertainty}
		...
		\begin{itemize}
			...
			\item A firm $i$ produces output $x$ ($x$ units of capital).\vspace{0.1in}
...
\end{frame}
		...
		\begin{frame}{Curse of dimensionality in this example}
			...
			\begin{enumerate}
				...
				\item Given intuition that individual $X_i$ have limited effect on $u(x,X)$, how to calculate $\expec{u(x',X')}$?\smallskip
					  \begin{itemize}
						  \item  Look at $\condexpec{u(x', X')}{w,\omega}$ to condition on firm's idiosyncratic $w$ aggregate shock $\omega$.\smallskip
...
\end{frame}
		...
		\begin{frame}[label=concentration]{Main result II: Concentration of measure}\label{Concentration}
			...
			\end{itemize}
			\emphcolor{Implication}: We can calculate $\condexpec{u(x', X')}{w, \omega}$ with a \textit{single draw} of idiosyncratic shocks $W$:
			\begin{itemize}
				\item $\condexpec{u(x', X')}{w, \omega}\approx u(x',X')$.\vspace{0.1in} 
...
\end{itemize}
   ...
   \end{enumerate}
	...
	% \end{frame}
			...
			\item The neural network ``learns'' very quickly that the solution is $u(x, X) = H_0 +\frac{1}{N}H_1\sum_{i=1}^N X_i$ and finds a high-dimensional approximation which matches that for the training grid.\vspace{0.1in}
...
\begin{frame}{Some challenging question: Generalization puzzle}
	\begin{itemize}
		...
		\end{itemize}
	\end{itemize}
\emphcolor{Question}: How come we achieve great generalization?  
\end{frame}

\begin{frame}{Some challenging questions: Stationarity and transversality puzzle}
	\begin{empheq}[box=\tcbhighmath]{align*}
...
\begin{itemize}
	...
	\begin{itemize}
		\item $H_1^- <0 \rightarrow$  \emphcolor{stationary} solution. \vspace{0.1in} 
		\item $H_1^+ > 0$ $\rightarrow$ \emphcolor{non-stationary} solution. \vspace{0.1in}
	...
	\end{itemize}
	\emphcolor{Question}: How come we never observe the non-stationary solution in the results? 
	\vspace{0.1in}
...
\begin{frame}{Answering the challenging questions}
	\begin{itemize}
		...
		\end{itemize}
	 \item Answering \emphcolor{stationarity puzzle}: In the linear set-up, the explosive solution has larger derivatives (less flat) than the non-explosive one i.e, $|H_1^+| > |H_1^-|$:\vspace{0.1in}
	 \begin{itemize}
...
\end{frame}				
		\section{Conclusions}
			...
			\begin{itemize}
				...
				\item Using \emphcolor{economic theory} and\emphcolor{deep learning} for function approximation with a huge \# of parameters ($\gg$ grid points)\vspace{0.1in}
			      \begin{itemize}
